...
import { Phone, Mail, MapPin, Users2, FileText, StickyNote, type LucideIcon } from "lucide-react";
import {
...
const PRIMARY = "#003366"; const STEEL = "#2D2D2D"; const BORDER = "#e5e7eb";
...
const INIT_CONTACTS: ContactEntry[] = [
  { id:1, date:"20 มิ.ย. 2569", icon:"call",    text:"โทรติดตามสถานะโครงการ — ได้รับการยืนยัน", type:"call" },
  { id:2, date:"12 มิ.ย. 2569", icon:"email",   text:"ส่งอีเมลเอกสารสัญญาฉบับปรับปรุง", type:"email" },
  { id:3, date:"5 มิ.ย. 2569",  icon:"meeting", text:"ประชุมหน้างาน — ตกลงเงื่อนไขการส่งมอบ", type:"meeting" },
  ...
  return (
    <div style={{ maxWidth:1100 }}>
      ...
      <div style={{ ...CARD, padding:"24px 28px", marginBottom:20 }}>
        ...
        {/* Info row */}
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(160px,1fr))", gap:12, marginTop:20, paddingTop:20, borderTop:"1px solid #e5e7eb" }}>
          {[
            { label:"โทรศัพท์", value:<span style={{display:"inline-flex",alignItems:"center",gap:5}}><Phone size={13} color="#6b7280" strokeWidth={2}/>{customer.phone}</span> },
            { label:"อีเมล",    value:<span style={{display:"inline-flex",alignItems:"center",gap:5}}><Mail size={13} color="#6b7280" strokeWidth={2}/>{customer.email}</span> },
            { label:"จังหวัด",  value:<span style={{display:"inline-flex",alignItems:"center",gap:5}}><MapPin size={13} color="#6b7280" strokeWidth={2}/>{customer.province}</span> },
      ...
      <div style={{ display:"flex", gap:2, marginBottom:16, background:"#f0f4f8", borderRadius:12, padding:4, width:"fit-content" }}>
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            style={{ padding:"8px 18px", borderRadius:9, border:"none", background:tab===t.key?"#fff":"transparent", color:tab===t.key?PRIMARY:"#6b7280", fontSize:"0.78rem", fontWeight:700, cursor:"pointer", boxShadow:tab===t.key?"0 1px 4px rgba(0,0,0,.12)":"none", transition:"all .12s", whiteSpace:"nowrap" }}>
            {t.label}
          </button>
        ))}
      </div>
                    ...
                    <div style={{ ...CARD, padding:"18px 20px", cursor:"pointer", transition:"transform .12s" }}
                      ...
                      <div style={{ marginBottom:10 }}>
                        ...
                        <div style={{ height:6, background:"#f0f0f5", borderRadius:99, overflow:"hidden" }}>
                          <div style={{ height:"100%", width:`${p.progress}%`, background:p.progress===100?"#059669":PRIMARY, borderRadius:99 }}/>
                        </div>
                      </div>
            ...
            {contacts.map((a, i) => (
              <div key={a.id} style={{ display:"flex", gap:14, position:"relative", paddingBottom:i<contacts.length-1?20:0 }}>
                ...
                <div style={{ width:30, height:30, borderRadius:"50%", background:"#dce5f0", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, zIndex:1 }}>
                  {(() => { const Icon = CONTACT_ICON[a.icon] ?? FileText; return <Icon size={14} color={PRIMARY} strokeWidth={2} />; })()}
                </div>
                <div style={{ flex:1 }}>
