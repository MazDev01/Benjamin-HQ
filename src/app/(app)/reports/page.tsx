    ...
    <table style={{width:"100%",borderCollapse:"collapse"}}>
      ...
      <tbody>
        {data.map((row,i)=>{
          ...
          return <tr key={i} style={{borderBottom:"1px solid #fafafa"}} onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background="#f8f9fb";}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background="";}}>
            ...
            <td style={{padding:"9px 10px",fontSize:"0.78rem",fontWeight:700,color:diff>=0?"#059669":"#dc2626"}}>{fmtDiff(diff)}</td>
        ...
        <tr style={{borderTop:`2px solid ${BORDER}`,background:"#f8f9fb"}}>
          ...
          <td style={{padding:"9px 10px",fontSize:"0.78rem",fontWeight:700,color:data.reduce((s,d)=>s+d.actual-d.plan,0)>=0?"#059669":"#dc2626"}}>{fmtDiff(data.reduce((s,d)=>s+d.actual-d.plan,0))}</td>
          <td/>
        </tr>
      </tbody>
    </table>
  ...
  return (
    <div onClick={onClick} style={{...CARD,padding:"16px 18px",display:"flex",alignItems:"center",gap:14,cursor:onClick?"pointer":"default",transition:"box-shadow .15s"}} onMouseEnter={e=>{if(onClick)(e.currentTarget as HTMLElement).style.boxShadow="0 4px 16px rgba(0,0,0,.14)";}} onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.boxShadow="0 2px 14px rgba(0,0,0,.07)";}}>
      ...
      <div>
        ...
        <div style={{fontSize:"0.68rem",marginTop:2,color:trendUp?"#059669":"#dc2626",fontWeight:600}}>{trendUp?"↑":"↓"} {trend}</div>
      </div>
    </div>
  );
}
    ...
    <div style={{fontFamily:"'Noto Sans Thai', sans-serif",paddingBottom:32}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:12,flexWrap:"wrap",gap:10}}>
        ...
        <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <button onClick={()=>setShowModal(true)}
            style={{display:"flex",alignItems:"center",gap:5,padding:"7px 15px",borderRadius:8,fontSize:"0.76rem",fontWeight:700,cursor:"pointer",border:"none",background:PRIMARY,color:"#fff",boxShadow:"0 4px 10px rgba(0,0,0,.25)"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}}>
        <StatCard iconBg="#e5faf0" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#059669" strokeWidth="2"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>}
          value={fmtB(totalActual)} valueColor="#059669" label="รายได้จริงในช่วงนี้" trend={`เกินแผน ${fmtB(Math.max(totalActual-totalPlan,0))}`} trendUp={totalActual>=totalPlan} onClick={()=>router.push("/payments")}/>
        <StatCard iconBg="#fee2e2" icon={<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#dc2626" strokeWidth="2"><path d="M20.84 4.61a5.5 5.5 0 00-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 00-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 000-7.78z"/></svg>}
          value={fmtB(totalPlan*0.72)} valueColor="#dc2626" label="ค่าใช้จ่าย (ประมาณ)" trend="72% ของแผน" trendUp={false} onClick={()=>router.push("/contracts")}/>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:12,marginBottom:14}}>
        {[
          {label:"เสร็จตามแผน",    val:onTimeRate,    suffix:"%", color:"#059669", bg:"#e5faf0", note:`${onTimeProjects.length}/${completedProjects.length} โครงการ`},
      ...
      <div style={{...CARD,padding:"16px 20px 18px",marginBottom:14}}>
        ...
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10,gap:10,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:2,background:"#fafafa",borderRadius:9,padding:"3px"}}>
            {(["year","6m","quarter","month","custom"] as Preset[]).map(p=>(
              <button key={p} onClick={()=>applyPreset(p)}
                style={{padding:"4px 11px",borderRadius:7,border:"none",background:preset===p?"#fff":"transparent",color:preset===p?PRIMARY:SUB,fontSize:"0.73rem",fontWeight:preset===p?700:500,cursor:"pointer",boxShadow:preset===p?"0 1px 4px rgba(0,0,0,.1)":"none",transition:"all .12s",whiteSpace:"nowrap"}}>
                {PRESET_LABELS[p]}
              </button>
            ))}
          </div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:"0.78rem"}}>
            ...
            <tbody>
              {projectHealth.map((p,i)=>{
                ...
                return (
                  <tr key={p.id} style={{borderBottom:i<projectHealth.length-1?`1px solid #fafafa`:"none",cursor:"pointer"}}
                    ...
                    <td style={{padding:"10px 14px",minWidth:120}}>
                      <div style={{display:"flex",alignItems:"center",gap:6}}>
                        <div style={{flex:1,height:5,background:"#f0f4f8",borderRadius:99,overflow:"hidden"}}>
                          <div style={{height:"100%",width:`${p.progress}%`,background:p.status==="completed"?"#059669":PRIMARY,borderRadius:99}}/>
                        </div>
                    ...
                    <td style={{padding:"10px 14px",fontWeight:700,fontSize:"0.74rem",color:p.daysLeft<0?"#dc2626":p.daysLeft<14?"#f59e0b":"#059669",whiteSpace:"nowrap"}}>
