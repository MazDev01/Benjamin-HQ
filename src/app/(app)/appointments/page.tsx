          ...
          <div style={{padding:"18px 22px 14px",borderBottom:`1px solid ${BORDER}`,display:"flex",alignItems:"flex-start",gap:12}}>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5,flexWrap:"wrap"}}>
                <span style={{padding:"3px 10px",borderRadius:99,fontSize:"0.68rem",fontWeight:700,background:tc.bg,color:tc.text,whiteSpace:"nowrap"}}>{apptTypeLabel[appt.type]}</span>
            ...
            <div style={{paddingTop:4}}>
              {!showDeleteConfirm?(
                <button onClick={()=>setShowDeleteConfirm(true)}
                  style={{display:"flex",alignItems:"center",gap:8,padding:"9px 14px",borderRadius:10,background:"#fff",border:`1px solid ${BORDER}`,color:"#dc2626",fontSize:"0.75rem",fontWeight:700,cursor:"pointer",width:"100%"}}>
...
function ApptCard({ a, onClick }:{ a:AppointmentMock; onClick:()=>void }){
  ...
  return (
    <div onClick={onClick}
      ...
      onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.borderColor="#C0C0C0";(e.currentTarget as HTMLElement).style.boxShadow="0 4px 18px rgba(0,0,0,.12)";}}
      onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.borderColor=BORDER;(e.currentTarget as HTMLElement).style.boxShadow="0 2px 14px rgba(0,0,0,.07)";}}>
      ...
      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5,flexShrink:0}}>
        <span style={{display:"inline-block",padding:"3px 10px",borderRadius:99,fontSize:"0.68rem",fontWeight:700,background:tc.bg,color:tc.text,whiteSpace:"nowrap"}}>{apptTypeLabel[a.type]}</span>
        <span style={{display:"inline-block",padding:"3px 10px",borderRadius:99,fontSize:"0.65rem",fontWeight:600,background:sc.bg,color:sc.text,whiteSpace:"nowrap"}}>{apptStatusLabel[a.status]}</span>
...
function CalendarSidebar({ calYear, calMonth, apptData, onCard }:{
  ...
  const dotColors: Record<ApptType,string> = {
    survey:"#003366",design_meet:"#003366",presentation:"#f59e0b",contract_sign:"#003366",handover:"#059669",follow_up:"#6b7280",
  };
  return (
      ...
      <div style={{...CARD,padding:16}}>
        ...
        {upcoming.length===0?(
          ...
          <div>{upcoming.map((a,idx)=>{
            ...
            const metaColor=isOverdue?"#dc2626":isWarn?"#f59e0b":MUTED;
            ...
            return(
              <div key={a.id} onClick={()=>onCard(a)} style={{display:"flex",gap:10,paddingTop:idx===0?0:10,paddingBottom:isLast?0:10,borderBottom:isLast?"none":`1px solid ${BORDER}`,cursor:"pointer"}}
                ...
                <div style={{width:10,height:10,borderRadius:"50%",background:isOverdue?"#dc2626":isWarn?"#f59e0b":dotColors[a.type],flexShrink:0,marginTop:3}}/>
                <div style={{flex:1,minWidth:0}}>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",marginBottom:16,flexWrap:"wrap",gap:12}}>
        <div>
          <h1 style={{fontSize:"1.55rem",fontWeight:800,color:STEEL,marginBottom:3}}>นัดหมาย</h1>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          ...
          <button onClick={()=>setShowAdd(true)}
            style={{display:"flex",alignItems:"center",gap:6,background:PRIMARY,color:"#fff",border:"none",borderRadius:10,padding:"8px 16px",fontSize:"0.78rem",fontWeight:700,cursor:"pointer",boxShadow:"0 4px 10px rgba(0,0,0,.25)"}}>
