...
function StyledInput({ value, onChange, placeholder, type = "text" }: { value: string; onChange: (v: string) => void; placeholder?: string; type?: string }) {
  return (
    <input
      ...
      style={inputStyle}
      onFocus={e => { e.currentTarget.style.borderColor = "#003366"; e.currentTarget.style.background = "#fff"; e.currentTarget.style.boxShadow = "0 0 0 3px rgba(0,0,0,.10)"; }}
...
function BtnPrimary({ onClick, children }: { onClick?: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick}
      style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#003366", color: "#fff", border: "none", borderRadius: 9, padding: "8px 16px", fontSize: "0.78rem", fontWeight: 700, cursor: "pointer", boxShadow: "0 3px 8px rgba(0,0,0,.22)", fontFamily: "inherit" }}>
      {children}
    </button>
  );
}
...
function BtnGhost({ onClick, children, danger }: { onClick?: () => void; children: React.ReactNode; danger?: boolean }) {
  return (
    <button onClick={onClick}
      style={{ display: "inline-flex", alignItems: "center", gap: 4, background: "transparent", color: danger ? "#dc2626" : "#6b7280", border: `1px solid ${danger ? "#dc2626" : "#e5e7eb"}`, borderRadius: 8, padding: "5px 10px", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
      {children}
    </button>
  );
}
    ...
    <div>
      ...
      <div style={{ padding: "20px 24px" }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ width: 100, height: 100, borderRadius: 12, border: "2px dashed #e5e7eb", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 6, cursor: "pointer", color: "#6b7280" }}>
...
const DEFAULT_STAGES: Stage[] = [
  { id: 1, name: "ผู้สนใจ", color: "#003366", pos: 1 },
  { id: 2, name: "ติดต่อแล้ว", color: "#f59e0b", pos: 2 },
  { id: 3, name: "สำรวจหน้างาน", color: "#8fa3b8", pos: 3 },
  { id: 4, name: "ส่งใบเสนอราคา", color: "#003366", pos: 4 },
  { id: 5, name: "เจรจาราคา", color: "#f59e0b", pos: 5 },
  { id: 6, name: "ปิดการขาย", color: "#059669", pos: 6 },
  { id: 7, name: "ไม่สนใจ", color: "#dc2626", pos: 7 },
];

const STAGE_COLORS = ["#003366", "#002244", "#059669", "#f59e0b", "#dc2626", "#8fa3b8", "#2D2D2D", "#C0C0C0"];
  ...
  return (
    <div>
      ...
      <div style={{ padding: "16px 24px" }}>
        {/* Add form */}
        {adding && (
          <div style={{ background: "#fafafa", borderRadius: 12, border: "1px solid #e5e7eb", padding: "14px 16px", marginBottom: 12, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            ...
            <tbody>
              {sorted.map((s, idx) => (
                <tr key={s.id}
                  ...
                  style={{ borderTop: "1px solid #f0f4f8", background: dragOverId === s.id ? "#dce5f0" : (dragId === s.id ? "#fafafa" : "#fff"), transition: "background .1s" }}>
                ...
                <tr key={t.id} style={{ borderTop: "1px solid #f0f4f8", background: "#fff" }}>
                  ...
                  <td style={{ padding: "10px 16px" }}>
                    <button onClick={() => toggleActive(t.id)}
                      style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 20, border: "none", cursor: "pointer", background: t.active ? "#e5faf0" : "#fee2e2", color: t.active ? "#059669" : "#dc2626", fontSize: "0.73rem", fontWeight: 700, fontFamily: "inherit" }}>
                      <span style={{ width: 7, height: 7, borderRadius: "50%", background: t.active ? "#059669" : "#dc2626", display: "inline-block" }} />
...
const DEPT_COLORS = ["#003366", "#002244", "#059669", "#f59e0b", "#8fa3b8", "#dc2626", "#2D2D2D", "#C0C0C0"];

const DEFAULT_DEPTS: Department[] = [
  { id: 1, name: "ฝ่ายขาย", color: "#003366", active: true },
  { id: 2, name: "วิศวกรรม", color: "#003366", active: true },
  { id: 3, name: "การเงิน", color: "#059669", active: true },
                ...
                <tr key={d.id} style={{ borderTop: "1px solid #f0f4f8", background: "#fff" }}>
                  ...
                  <td style={{ padding: "10px 16px" }}>
                    <button onClick={() => toggleActive(d.id)}
                      style={{ display: "inline-flex", alignItems: "center", gap: 5, padding: "4px 10px", borderRadius: 20, border: "none", cursor: "pointer", background: d.active ? "#e5faf0" : "#fee2e2", color: d.active ? "#059669" : "#dc2626", fontSize: "0.73rem", fontWeight: 700, fontFamily: "inherit" }}>
                      <span style={{ width: 7, height: 7, borderRadius: "50%", background: d.active ? "#059669" : "#dc2626", display: "inline-block" }} />
...
export default function SettingsPage() {
  ...
  return (
    <div>
      <div style={{ marginBottom: 20 }}>
        <h1 style={{ fontSize: "1.55rem", fontWeight: 800, color: "#2D2D2D", marginBottom: 3 }}>ตั้งค่าระบบ</h1>
