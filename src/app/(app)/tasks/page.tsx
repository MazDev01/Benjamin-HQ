...
function KanbanCard({task,onSelect,selected,onDragStart,onDragEnd,isDragging}:{
  ...
  return (
    <div onClick={onSelect}
      ...
      onDragEnd={onDragEnd}
      style={{background:"#fff",border:`1px solid ${selected?PRIMARY:BORDER}`,borderRadius:10,padding:"10px 12px",marginBottom:8,cursor:"grab",boxShadow:selected?"0 0 0 2px rgba(0,0,0,.18)":"0 1px 4px rgba(0,0,0,.05)",transition:"box-shadow .15s, opacity .15s",opacity:isDragging?0.4:isDone?0.55:1}}
  ...
  return (
    <>
      ...
      <div style={{position:"fixed",inset:0,zIndex:210,display:"flex",alignItems:"center",justifyContent:"center",padding:20,pointerEvents:"none"}}>
        <div onClick={e=>e.stopPropagation()} style={{...CARD,width:"100%",maxWidth:480,pointerEvents:"auto",overflow:"hidden",boxShadow:"0 24px 80px rgba(0,0,0,.2)"}}>
    ...
    <div style={{width:330,minWidth:300,flexShrink:0,...CARD,borderRadius:16,overflowY:"auto",maxHeight:"calc(100vh - 140px)",alignSelf:"flex-start",position:"sticky",top:0}}>
      ...
      <div style={{background:PRIMARY,padding:"16px 14px 12px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
          <span style={{padding:"2px 10px",borderRadius:99,fontSize:"0.63rem",fontWeight:700,background:sb.bg,color:sb.text,whiteSpace:"nowrap"}}>{taskStatusLabel[task.status]}</span>
          <PriorityBadge priority={task.priority}/>
        </div>
      </div>
      {/* Tabs */}
            ...
            <>
              <button onClick={()=>router.push("/projects")}
                ...
                <div style={{flex:1}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    <span style={{padding:"2px 8px",borderRadius:99,fontSize:"0.62rem",fontWeight:700,background:projectStatusColor[relProject.status].bg,color:projectStatusColor[relProject.status].text,whiteSpace:"nowrap"}}>{projectStatusLabel[relProject.status]}</span>
