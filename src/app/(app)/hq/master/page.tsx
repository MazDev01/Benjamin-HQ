...
const PRIMARY = "#003366"; const STEEL = "#2D2D2D"; const BORDER = "#e5e7eb"; const SUCCESS = "#059669";
...
const PRICE_DATA: Record<string, PriceRow[]> = {
  ...
  CUSTOM: [
    { code:"CT-S01",  name:"งานโครงสร้างเหล็กตามแบบ",            unit:"ตัน",    price:"42,000",  vip:"39,000",  updated:"1 มิ.ย. 2569" },
    { code:"CT-C01",  name:"งานคอนกรีตเสริมเหล็ก (ฐานราก)",      unit:"ลบ.ม.",  price:"3,200",   vip:"3,000",   updated:"1 มิ.ย. 2569" },
    { code:"CT-A01",  name:"งานสถาปัตย์-ตกแต่งภายใน",            unit:"ตร.ม.",  price:"4,500",   vip:"4,200",   updated:"15 พ.ค. 2569" },
    { code:"CT-M01",  name:"งานระบบไฟฟ้า-ประปา (MEP)",          unit:"จุด",    price:"1,800",   vip:"1,650",   updated:"15 พ.ค. 2569" },
  ],
  TURNKEY: [
    { code:"TK-W01",  name:"คลังสินค้าครบวงจร (Design & Build)", unit:"ตร.ม.",  price:"8,500",   vip:"8,000",   updated:"1 มิ.ย. 2569" },
    { code:"TK-F01",  name:"โรงงานครบวงจร (Design & Build)",     unit:"ตร.ม.",  price:"12,000",  vip:"11,200",  updated:"1 มิ.ย. 2569" },
    { code:"TK-Y01",  name:"ลานคอนกรีต / ถนนภายในโครงการ",       unit:"ตร.ม.",  price:"950",     vip:"880",     updated:"15 พ.ค. 2569" },
    { code:"TK-U01",  name:"งานสาธารณูปโภค (ไฟฟ้า/ประปา/บำบัด)", unit:"เหมา",   price:"ตามขอบเขตงาน", vip:"—",  updated:"1 มิ.ย. 2569" },
  ],
  CONSULTANT: [
    { code:"CS-A01",  name:"งานออกแบบสถาปัตยกรรม",              unit:"โครงการ",price:"ตามข้อตกลง",vip:"—",   updated:"1 มิ.ย. 2569" },
    { code:"CS-E01",  name:"งานออกแบบวิศวกรรมโครงสร้าง",         unit:"โครงการ",price:"ตามข้อตกลง",vip:"—",   updated:"1 มิ.ย. 2569" },
    { code:"CS-P01",  name:"บริหารโครงการ (PM)",                unit:"เดือน",  price:"85,000",  vip:"80,000",  updated:"1 มิ.ย. 2569" },
    { code:"CS-C01",  name:"ที่ปรึกษา/ตรวจสอบงานก่อสร้าง",       unit:"ครั้ง",  price:"15,000",  vip:"13,500",  updated:"1 มิ.ย. 2569" },
  ],
};

const PRODUCT_LINES = [
  { name:"EASYBUILD",  desc:"รับสร้างโกดังสำเร็จรูป",       items:PRICE_DATA.EASYBUILD.length,  status:"active" },
  { name:"RANBUILD",   desc:"รับสร้างอาคาร",                items:PRICE_DATA.RANBUILD.length,   status:"active" },
  { name:"PREFAB",     desc:"อาคารสำเร็จรูปทุกประเภท",      items:PRICE_DATA.PREFAB.length,     status:"active" },
  { name:"CUSTOM",     desc:"ก่อสร้างตามแบบลูกค้า",         items:PRICE_DATA.CUSTOM.length,     status:"active" },
  { name:"TURNKEY",    desc:"คลังสินค้า/โรงงานครบวงจร",     items:PRICE_DATA.TURNKEY.length,    status:"active" },
  { name:"CONSULTANT", desc:"ออกแบบและให้คำปรึกษา",         items:PRICE_DATA.CONSULTANT.length, status:"active" },
];

const LINE_COLORS: Record<string,string> = {
  EASYBUILD:"#003366", RANBUILD:"#0a4f8c", PREFAB:"#1e6fbf",
  CUSTOM:"#4d94d4", TURNKEY:"#82b4e3", CONSULTANT:"#b8d4f0",
};
...
export default function HQMasterPage() {
  ...
  function exportCsv() {
    const headers = ["รหัสสินค้า","รายการ","หน่วย","ราคา (บาท)","ราคา VIP (บาท)","อัปเดตล่าสุด"];
    const body = rows.map(r => [r.code, r.name, r.unit, r.price, r.vip, r.updated]);
    ...
    a.href = url;
    a.download = `ราคากลาง-${selectedLine}.csv`;
    a.click();
  ...
  return (
    <div>
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:16 }}>
        <div>
          <h1 style={{ fontSize:"1.55rem", fontWeight:800, color:STEEL, marginBottom:3 }}>ราคากลาง</h1>
        ...
        {PRODUCT_LINES.map(p => {
          ...
          return (
            <div key={p.name} onClick={() => setSelectedLine(p.name)}
              style={{ ...CARD, padding:"18px 18px 14px", cursor:"pointer", border:`2px solid ${selected?LINE_COLORS[p.name]:"#e5e7eb"}`, transition:"border-color .15s" }}>
      ...
      {/* Price table */}
      <div style={CARD}>
        <div style={{ padding:"14px 16px", borderBottom:"1px solid #e5e7eb", display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8 }}>
          <div>
          ...
          <div style={{ display:"flex", gap:8 }}>
            ...
            <button onClick={exportCsv}
              style={{ display:"flex", alignItems:"center", gap:6, background:STEEL, color:"#fff", border:"none", borderRadius:9, padding:"7px 16px", fontSize:"0.76rem", fontWeight:700, cursor:"pointer" }}>
              ↓ ส่งออก CSV
            </button>
          </div>
        </div>
                ...
                <tr key={row.code} style={{ borderBottom:"1px solid #f0f4f8" }}
                  ...
                  <td style={{ padding:"11px 14px" }}>
                    <button onClick={() => deleteRow(row.code)}
                      style={{ padding:"3px 10px", borderRadius:7, border:"1px solid #fee2e2", background:"#fff", color:"#dc2626", fontSize:"0.7rem", fontWeight:600, cursor:"pointer" }}>ลบ</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {showAdd && (
        <>
          ...
          <div style={{ position:"fixed", top:"50%", left:"50%", transform:"translate(-50%,-50%)", zIndex:201,
            ...CARD, padding:"24px 28px", width:420, boxShadow:"0 16px 48px rgba(0,0,0,.18)" }}>
